...
export SLAVE_IP_ADDRESS=$(ip addr show eth0 | grep 'inet ' | cut -d'/' -f 1 | cut -c '10-')
...
(
  ...
  do
    ...
    /usr/local/sbin/mesos-slave --master=zk://zookeeper:2181/mesos/master --ip=$SLAVE_IP_ADDRESS \
